...
use der_parser::der::{
    der_read_element_header, parse_der_ia5string, parse_der_null, parse_der_octetstring,
    parse_der_oid, parse_der_sequence_defined_g, parse_der_sequence_of,
};
...
impl BIMICertificate {
    ...
    pub fn from_pem(input: &[u8], ca_storage: &CAStorage) -> Result<Self, AppError> {
        ...
        while let Some(cert) = x509_stack.pop() {
            if !x509_is_ca(&cert) {
                ...
                ca_storage.try_add_ca_cert(&cert).unwrap_or_else(|e| {
    ...
    pub fn verify_expiry(&self) -> bool {
        let now = SystemTime::now()
            ...
            .expect("cannot run if clock are before unix epoch")
            .as_secs() as i64;
...
fn verify_subject_name(domain: &str, x509_name: &X509NameRef) -> Result<(), AppError> {
    if let Some(pat) = x509_name.entries_by_nid(Nid::COMMONNAME).next() {
        let pattern = pat
            ...
            .map_err(|_| {
                AppError::CertificateNameVerificationError("bad subject name".to_string())
            })?;
        match domain.parse::<IpAddr>() {
            Ok(_) => Err(AppError::CertificateNameVerificationError(
                "IP address in subject name".to_string(),
            )),
            Err(_) => verify_dns(domain, pattern.as_str()),
        }
    } else {
        Err(AppError::CertificateNameVerificationError(
            "no subject names".to_string(),
        ))
    }
}

/// Verifies a stack of alt names
fn verify_subject_alt_names(domain: &str, names: &Stack<GeneralName>) -> Result<(), AppError> {
    for name in names {
        match name.dnsname() {
            Some(n) => {
                if verify_dns(domain, n).is_ok() {
                    // If any name matches, we assume it as a successful match
                    return Ok(());
                }
            }
            _ => {
                return Err(AppError::CertificateNameVerificationError(
                    "Invalid alt name".to_string(),
                ))
            }
        }
    }
    ...
    Err(AppError::CertificateNameVerificationError(
        "No matching alt name".to_string(),
    ))
}
...
fn verify_dns(domain: &str, pattern: &str) -> Result<(), AppError> {
    ...
    let domain_to_check = domain.strip_suffix('.').unwrap_or(domain);
    let pattern_to_check = pattern.strip_suffix('.').unwrap_or(pattern);

    // Check either wildcard definitions of just the whole name
    let wildcard_location = match pattern_to_check.find('*') {
        Some(positions) => positions,
        None => {
            if domain_to_check == pattern_to_check {
                return Ok(());
            } else {
                return Err(AppError::CertificateNameVerificationError(
                    "Cannot verify domain name".to_string(),
                ));
            }
        }
    };

    let mut dot_idxs = pattern_to_check.match_indices('.').map(|(pos, _)| pos);
    ...
    let first_label_pos = match domain_to_check.find('.') {
        ...
        None => {
            return Err(AppError::CertificateNameVerificationError(
                "cannot match single label with wildcard".to_string(),
            ));
        }
    };
    ...
    if pattern_to_check[wildcard_end..] != domain_to_check[first_label_pos..] {
        return Err(AppError::CertificateNameVerificationError(
            "cannot match wildcard".to_string(),
        ));
    }
...
fn check_pem(input: &[u8]) -> bool {
    HEADER_SRCH
        .search_in(input)
        .and_then(|s| {
            FOOTER_SRCH.search_in(input).and_then(
                |e| {
                    if s + 1 >= e - 1 {
                        None
                    } else {
...
pub fn process_cert(
    ...
    let cert = BIMICertificate::from_pem(input, ca_storage)?;
    ...
    let image_vec =
        x509_bimi_get_ext(&cert.certificate).ok_or(AppError::CertificateNoLogoTypeExt)?;
    ...
    if image_url.starts_with("data:") {
        ...
        let image_url =
            DataUrl::process(image_url).map_err(|_| AppError::CertificateInvalidLogoURL)?;
...
fn parse_logotype_ext(input: &[u8]) -> Result<&str, AppError> {
    ...
    let first_url = &urls.as_sequence()?[0].as_sequence()?[0].as_sequence()?[0];
...
#[cfg(test)]
mod test {
    use crate::cert::{process_cert, verify_dns};
    use crate::mini_pki::CAStorage;
    ...
    fn verify_assets() {
        ...
        // Expired cert
        process_cert(
            &read_bytes("test-assets/paypal.pem").unwrap(),
            &ca_storage,
            "paypal.com",
        )
        .unwrap_err();
        ...
        .unwrap_err();
        process_cert(
            &read_bytes("test-assets/valimail.pem").unwrap(),
            &good_ca_storage,
            "valimail.com",
        )
        .unwrap();
